...
class HrPayslip(models.Model):
    ...
    compania_padre = fields.Many2one('res.company', 'Compañia padre')
            ...
            for work_entry_type_id, hours in work_hours_ordered:
                ...
                if work_entry_type.code != 'WORK100':
                    if contract.work_entry_source == 'calendar':
                       if work_entry_type.code != 'OVERTIME':
...
#                payslip_total_TDED += round(self.importe_imss,2)
        ...
        request_params.update({
                'factura': {
                      'serie': self.compania_padre.serie_nomina,
                   ...
                   #   'forma_pago': self.forma_pago,
                      ...
                      'LugarExpedicion': self.compania_padre.zip,
                      'RegimenFiscal': self.compania_padre.regimen_fiscal,
                ...
                'emisor': {
                      'rfc': self.compania_padre.vat,
                      'nombre': self.compania_padre.nombre_fiscal.upper(),
                },
                ...
                'conceptos': concepto,
                'nomina12Emisor': {'RfcPatronOrigen': self.compania_padre.vat}, ########este duplicado
                'nomina12': {
                      ...
                      'Receptor': {
                            ...
                            'SalarioBaseCotApor': self.set_decimals(self.contract_id.sueldo_base_cotizacion, 2) if contrato not in ['09', '10', '99'] else '',
                            'SalarioDiarioIntegrado': self.set_decimals(self.contract_id.sueldo_diario_integrado, 2) if contrato not in ['09', '10', '99'] else '',
                      },
                },
                'informacion': {
                      'cfdi': '4.0',
                      'sistema': 'odoo17 EE',
                      'version': '2',
                      'api_key': self.compania_padre.proveedor_timbrado,
                      'modo_prueba': self.compania_padre.modo_prueba,
                },
        })
...
#****** CERTIFICADOS *******
        if not self.compania_padre.archivo_cer:
            raise UserError(_('Archivo .cer path is missing.'))
        if not self.compania_padre.archivo_key:
            raise UserError(_('Archivo .key path is missing.'))
        archivo_cer = self.compania_padre.archivo_cer
        archivo_key = self.compania_padre.archivo_key
        request_params.update({
                'certificados': {
                      'archivo_cer': '', #archivo_cer.decode("utf-8"),
                      'archivo_key': '', #archivo_key.decode("utf-8"),
                      'contrasena': '', #self.compania_padre.contrasena,
                }})
    ...
    def action_cfdi_nomina_generate(self):
        for payslip in self:
            ...
            if payslip.compania_padre.proveedor_timbrado == 'servidor':
                url = '%s' % ('https://facturacion.itadmin.com.mx/api/nomina')
            elif payslip.compania_padre.proveedor_timbrado == 'servidor2':
...
#            _logger.info('something ... %s', response.text)
    ...
    def action_cfdi_cancel(self):
        for payslip in self:
            if payslip.nomina_cfdi:
                ...
                if not payslip.compania_padre.archivo_cer:
                    raise UserError(_('Falta la ruta del archivo .cer'))
                if not payslip.compania_padre.archivo_key:
                    raise UserError(_('Falta la ruta del archivo .key'))
                archivo_cer = payslip.compania_padre.archivo_cer
                archivo_key = payslip.compania_padre.archivo_key
                domain = [
                     ('res_id', '=', payslip.id),
                     ('res_model', '=', payslip._name),
                ...
                values = {
                          'rfc': payslip.compania_padre.vat,
                          'api_key': payslip.compania_padre.proveedor_timbrado,
                          'uuid': payslip.folio_fiscal,
                          'folio': payslip.number_folio,
                          'serie_factura': payslip.compania_padre.serie_nomina,
                          'modo_prueba': payslip.compania_padre.modo_prueba,
                            'certificados': {
                                 # 'archivo_cer': archivo_cer.decode("utf-8"),
                                 # 'archivo_key': archivo_key.decode("utf-8"),
                                  'contrasena': payslip.compania_padre.contrasena,
                            },
    ...
    def compute_sheet(self):
        for invoice in self:
            ...
            invoice.compania_padre = invoice.company_id.sudo().parent_ids[::-1].filtered(lambda x: x.estado_csd == 'Certificados CSD correctos')[:1]
            if not invoice.compania_padre:
                invoice.compania_padre = invoice.company_id.id
...
#            rec.refresh()
    ...
    def read_xml_report_values(self):
        for payslip in self:
            domain = [('res_id', '=', payslip.id),
                      ('res_model', '=', payslip._name),
                      ('name', '=', payslip.number.replace('/', '_') + '.xml')]
            xml_file = payslip.env['ir.attachment'].search(domain, limit=1)
            if not xml_file:
                ...
                attch_file = base64.b64decode(xml_file.datas)
                xml_data = etree.fromstring(attch_file)
                NSMAP = {
                     'xsi':'http://www.w3.org/2001/XMLSchema-instance',
                     'cfdi':'http://www.sat.gob.mx/cfd/4', 
                     'tfd': 'http://www.sat.gob.mx/TimbreFiscalDigital',
